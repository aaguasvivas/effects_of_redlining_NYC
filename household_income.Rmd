...
STATEN ISLAND HOUSEHOLD INCOME


```{r staten-island-income-1950, include=FALSE}
...
staten_island_data1950 <- read.csv(household_income_data_1950) %>%
  filter(STATE == "New York" & B0F001 > 0) %>%
  mutate(MedianHHIncome1949 = B0F001) %>%
  select(GISJOIN,MedianHHIncome1949)
...
staten_island_tracts1950 <- sf::st_read(dsn = census_tract_1950,
                                layer = census_tract_layers_1950)
...
staten_island_tracts1950 <-
  staten_island_tracts1950 %>% filter(NHGISST == "360" & COUNTY == "085") %>%
  merge(staten_island_data1950, "GISJOIN")


# Set projection of tracts dataset to `projection` required by leaflet

staten_island_tracts1950 <- sf::st_transform(staten_island_tracts1950,  crs="+init=epsg:4326")

# Condense size of data for faster processing

staten_island_tracts1950 <- rmapshaper::ms_simplify(staten_island_tracts1950)

# Set palette color

write_rds(staten_island_tracts1950, 
          "shiny/household_income/staten_island/1950/household_income_1950.rds")
...
leaflet(staten_island_tracts1950) %>%
  addTiles() %>%
  addPolygons(stroke = FALSE, smoothFactor = 0.3, fillOpacity = 1,
              fillColor = ~pal(MedianHHIncome1949)) %>%
  addLegend(pal = pal, values = ~MedianHHIncome1949, opacity = 1.0)
...
```{r staten-island-income-1960, include=FALSE}
...
staten_island_household_income_1960 <- tracts1960Print %>%
  filter(STATE == "New York" & COUNTYA == "85" & !is.na(MedianIncome1960)) %>%
  select(GISJOIN, MedianIncome1960)

staten_island_1960_map <- sf::st_read(dsn = census_tract_1960,
                        layer = census_tract_layers_1960)

staten_island_1960_map <- staten_island_1960_map %>%
  merge(staten_island_household_income_1960, "GISJOIN")
...
write_rds(staten_island_1960_map, "shiny/household_income/staten_island/1960/household_income_1960.rds")
...
```{r staten-island-income-1970, include=FALSE}
...
staten_island_household_income_1970 <- tracts1970Print %>%
  filter(STATE == "New York" & COUNTYA == "85" & !is.na(MedianIncome1970)) %>%
  select(GISJOIN, MedianIncome1970)

staten_island_1970_map <- sf::st_read(dsn = census_tract_1970,
                        layer = census_tract_layers_1970)

staten_island_1970_map <- staten_island_1970_map %>%
  merge(staten_island_household_income_1970, "GISJOIN")
...
write_rds(staten_island_1970_map, "shiny/household_income/staten_island/1970/household_income_1970.rds")
...
```{r staten-island-income-1980, include=FALSE}
...
tracts1980 <-
  ...
  merge(staten_island_data1980, "GISJOIN")
...
write_rds(tracts1980, "shiny/household_income/staten_island/1980/household_income_1980.rds")
...
```{r staten-island-income-1990, include=FALSE}
...
write_rds(tracts1990, "shiny/household_income/staten_island/1990/household_income_1990.rds")
...
```{r staten-island-income-2000, include=FALSE}
...
write_rds(tracts2000, "shiny/household_income/staten_island/2000/household_income_2000.rds")
...
```{r staten-island-income-2010, include=FALSE}
...
write_rds(tracts2010, "shiny/household_income/staten_island/2010/household_income_2010.rds")
